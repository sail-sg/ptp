...
def create_dataset(dataset, config, min_scale=0.5):
    ...
    if dataset=='pretrain':
        if 'mixup' in config:
            ...
            mixup = False
        dataset = pretrain_dataset(config['train_file'], config['laion_path'], transform_train, img_mixup=mixup)
        return dataset  
    elif dataset=='caption_cc3m':
        dataset = pretrain_dataset(config['train_file'], config['laion_path'], transform_test, img_mixup=False)
        return dataset  
